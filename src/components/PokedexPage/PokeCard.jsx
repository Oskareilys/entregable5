...
const PokeCard = ({ pokeInfo }) => {
    ...
    const idPoke = pokeInfo.url.split('/')
  
    const navigate = useNavigate()
    

   const handlePokeDetail = () => {
      navigate(`/pokedex/${idPoke[6]}`)
    }

  return (
    <article className={`card ${pokemon?.types[0].idPoke}-border`} onClick={handlePokeDetail}>
      <header className={`card__header ${pokemon?.types[0].type.name}`}>
        <img className='card__img' src={pokemon?.sprites.other['official-artwork'].front_default} alt="" />
      </header>
      <section className='card__principal'>
        <h3 className={`card__name ${pokemon?.types[0].type.name}-color`}>{pokemon?.name}</h3>
        ...
        <div className='card__typetext'>Type</div>
      </section>
